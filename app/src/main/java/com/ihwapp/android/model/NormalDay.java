...
public class NormalDay extends Day {
    ...
    public void fillPeriods(Curriculum c) {
        ...
        for (int num = 1; num <= periodsBeforeBreak; num++) {
            ...
            if (course == null)
                periods.add(new Period("X", date, nextStart, nextStart.timeByAdding(duration), num, index, true));
            else
                periods.add(new Period(course.getName(), date, nextStart, nextStart.timeByAdding(duration), num, index, false));
